...
import Language.Iris.Parsing.Internal.SourceText
  ( ignoredTokens,
    ignoredTokens1,
    parseStringBS,
  )
...
setOf entry = braces (entry `sepEndBy` ignoredTokens) >>= lift . fromElems
{-# INLINE setOf #-}
...
parseRequired = (symbol QUESTION_MARK $> False) <|> pure True
{-# INLINE parseRequired #-}
