...
module Language.Iris.Parsing.Internal.Internal
  ( Parser,
    Position,
    getLocation,
    processParser,
...
type Parser = ParsecT MyError ByteString (Result GQLError)

type ErrorBundle = ParseErrorBundle ByteString MyError

processParser :: Parser a -> ByteString -> Result GQLError a
